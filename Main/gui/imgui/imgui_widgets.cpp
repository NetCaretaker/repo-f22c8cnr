...
void ImGui::Combo( const char* label, int* v, std::vector< const char* > items ) {
    ...
    obj.anim = anim( obj.anim, 0.f, 1.f, obj.open || obj.hover );
    ...
    RenderFrame( bb.Min, bb.Max, GetColorU32( ImGuiCol_FrameBg, 0 ), 1, style.FrameRounding );
    ...
    window->DrawList->AddText( bb.Min + style.FramePadding, obj.col, items[*v], FindRenderedTextEnd( items[*v] ) );
        ...
        Begin( label, 0, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoBackground );
        {
            ...
            BeginGroup( );
            {
                for ( int i = 0; i < items.size( ); ++i ) {
                    if ( Selectable( items[i], i == *v, 0, { GetWindowWidth( ), GetFrameHeight( ) } ) ) {
                        *v = i;
                        obj.open = false;
                    }
                }
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    struct s {
        ...
        float held;
        ImColor col;
    }; auto& obj = anim_obj( label, 2, s{ 0.f, 0.f, 0 } );
...
bool ImGui::Checkbox(const char* label, bool* v, c_key* key, float col[4])
{
    auto window = GetCurrentWindow( );
	auto id = window->GetID( label );
	auto style = GetStyle( );

	const float toggle_w = 34.0f;
	const float toggle_h = 18.0f;
	const float knob_r = 6.5f;
	const float knob_pad = 2.5f;
	ImVec2 pos = window->DC.CursorPos;
	ImVec2 label_size = CalcTextSize( label, 0, 1 );
	ImRect bb( pos, pos + ImVec2( CalcItemWidth( ), ImMax(toggle_h, label_size.y) ) );
    ImRect click_bb( { bb.Max.x - toggle_w, bb.GetCenter( ).y - toggle_h / 2 }, { bb.Max.x, bb.GetCenter( ).y + toggle_h / 2 } );
    ...
    struct s {
        ...
        ImColor col = 0;
        float bind_w = 0;
        ImColor circle_col = 0;
        float circle_anim = 0.f;
    ...
    obj.col = col_anim( GetColorU32( ImGuiCol_TextDisabled ), GetColorU32( ImGuiCol_Text ), obj.anim );
    obj.circle_col = col_anim( ImColor(180, 180, 190, 255), ImColor(255, 255, 255, 255), obj.enabled );
    obj.circle_anim = anim2( label, 0, obj.circle_anim, { 0.f, 1.4f, 1.f, 1.2f, 1.f, 1.1f, 1.f }, *v );
    ...
    if ( col ) {
        ...
        window->DC.CursorPos = ImVec2{ click_bb.Min.x - 24 - 10, bb.Min.y };

        BeginChild( std::string( "color" ).append( label ).c_str( ), { 24, toggle_h }, 0, ImGuiWindowFlags_NoBackground );
        ui::color_btn( std::string( label ).append( "cb" ).c_str( ), col, { 24, toggle_h } );
        EndChild( );
    ...
    if ( key ) {
        ...
        obj.bind_w = ImLerp( obj.bind_w, ui::text_size( fontb, 12, KEY_NAMES[key->key] ).x, GetIO( ).DeltaTime * 18.f );

        window->DC.CursorPos = ImVec2{ click_bb.Min.x - 10 - ( col ? 34 : 0 ) - obj.bind_w - 20, bb.GetCenter( ).y - 7 - 6 };

        PushFont( fonts[fontb].get( 12 ) );
        BeginChild( std::string( "bind" ).append( label ).c_str( ), { obj.bind_w + 20, 0 }, 0, ImGuiWindowFlags_NoBackground );
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    struct s {
        ...
        float anim;
        ImColor col;
    }; auto& obj = anim_obj( label, 1, s{ 0.f, 0.f, 0.f, 0 } );

    obj.rad = anim( obj.rad, IM_PI / 2, IM_PI * 1.5f, popup_open );
    obj.hover = anim( obj.rad, 0.f, 1.f, hovered );
    obj.popup_anim = anim( obj.rad, 0.f, 1.f, popup_open );
    obj.anim = anim( obj.rad, 0.f, 1.f, hovered || popup_open );
    obj.col = col_anim( GetColorU32( ImGuiCol_TextDisabled ), GetColorU32( ImGuiCol_Text ), obj.anim );
	...
	window->DrawList->AddRect( bb.Min, bb.Max, GetColorU32( ImGuiCol_Border ), style.FrameRounding, popup_open ? ImDrawFlags_RoundCornersTop : 0 );
    ...
    if ( preview_value )
        window->DrawList->AddText( bb.Min + style.FramePadding, obj.col, preview_value, FindRenderedTextEnd( preview_value ) );
    ...
    if ( popup_open ) {
        ...
        GetWindowDrawList( )->AddRect( GetWindowPos( ) - ImVec2{ 0, 1 }, GetWindowPos( ) + GetWindowSize( ), GetColorU32( ImGuiCol_Border ), style.PopupRounding, ImDrawFlags_RoundCornersBottom );

        GImGui->CurrentWindow->SizeFull.x = bb.GetWidth( );
        GImGui->CurrentWindow->SizeFull.y = g.NextWindowData.SizeConstraintRect.GetHeight( ) * obj.popup_anim;
    }
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    ImVec2 pos = window->DC.CursorPos;
    ImVec2 label_size = CalcTextSize(label, 0, 1);
    ImRect total_bb(pos, pos + ImVec2(CalcItemWidth(), label_size.y + style.ItemInnerSpacing.y + 6 + 10));
    ...
    }; auto& obj = anim_obj(label, 3, slider_struct{ 0.f, 0.f, 0 });

    bool value_changed = false;

    if (obj.input_active) {
        total_bb = ImRect(pos, pos + ImVec2{ total_bb.GetWidth(), label_size.y + 10 + GetFrameHeight() });
        bb = ImRect(total_bb.Max - ImVec2{ total_bb.GetWidth(), GImGui->FontSize }, total_bb.Max);

        if (!IsMouseHoveringRect(total_bb.Min, total_bb.Max) && IsMouseClicked(0) || IsKeyPressed(ImGuiKey_Enter) || IsKeyPressed(ImGuiKey_Escape)) obj.input_active = false;

        PushStyleVar(ImGuiStyleVar_ItemSpacing, { 0, 10 });
        PushStyleColor(ImGuiCol_FrameBg, GetColorU32(ImGuiCol_FrameBg, 0));
        TextEx(label, FindRenderedTextEnd(label), ImGuiTextFlags_NoWidthForLargeClippedText);
        char buf[8];
        DataTypeFormatString(buf, IM_ARRAYSIZE(buf), data_type, p_data, format);
        ImStrTrimBlanks(buf);
        if (InputTextEx("##1234", "", buf, sizeof(buf), { 0, 0 }, 0, 0, 0)) {
            value_changed = true;
        ...
        if (clicked && IsKeyDown(ImGuiKey_LeftCtrl)) obj.input_active = true;
        ...
        ImRect grab_bb;
        value_changed = SliderBehavior(bb, id, data_type, p_data, p_min, p_max, format, NULL, &grab_bb);
        if (value_changed)
            MarkItemEdited(id);
        ...
        obj.anim = anim(obj.anim, 0.f, 1.f, hovered || IsItemActive());
        obj.col = col_anim(GetColorU32(ImGuiCol_TextDisabled), GetColorU32(ImGuiCol_Text), obj.anim);
        obj.val_anim = ImLerp(obj.val_anim, grab_bb.Max.x - bb.Min.x, 17.f * GetIO().DeltaTime);

        window->DrawList->AddRectFilled(total_bb.Min, total_bb.Max, GetColorU32(ImGuiCol_FrameBg), 3);

        window->DrawList->AddRect(bb.Min, bb.Max, ImGui::GetColorU32(ImGuiCol_Border), 2);
        window->DrawList->AddRectFilled(bb.Min, { bb.Min.x + obj.val_anim * ui::content_anim * ui::content_anim2, bb.Max.y }, GetColorU32(ImGuiCol_Scheme), 2);

        window->DrawList->AddText({ total_bb.Min.x + 8, total_bb.GetCenter().y - label_size.y / 2 }, obj.col, label, FindRenderedTextEnd(label));
        window->DrawList->AddText({ total_bb.Max.x - CalcTextSize(value_buf).x - 8, total_bb.GetCenter().y - label_size.y / 2 }, GetColorU32(ImGuiCol_Text), value_buf);
    }
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    struct s {
        ...
        float selected;
        ImColor col;
    }; auto& obj = anim_obj( label, 0, s{ 0.f, 0.f, 0.f, 0 } );

    obj.anim = anim( obj.anim, 0.f, 1.f, hovered || selected );
    obj.hover = anim( obj.hover, 0.f, 1.f, hovered );
    obj.selected = anim( obj.selected, 0.f, 1.f, selected );
    obj.col = col_anim( col_anim( GetColorU32( ImGuiCol_TextDisabled ), GetColorU32( ImGuiCol_Text ), obj.hover ), GetColorU32( ImGuiCol_Scheme ), obj.selected );

    window->DrawList->AddRectFilled( bb.Min, bb.Max, ImColor{ 1.f, 1.f, 1.f, 0.03f * obj.hover * style.Alpha } );

    window->DrawList->AddText( { bb.Min.x + style.FramePadding.x, bb.GetCenter( ).y - label_size.y / 2 }, obj.col, label );
